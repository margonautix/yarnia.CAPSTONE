    ...
    <div className="min-h-screen bg-surface dark:bg-surface-dark text-primary dark:text-primary-dark px-4 sm:px-6 lg:px-8 py-10">
      <div className="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-[250px_1fr] gap-6">
        ...
        <aside className="bg-card dark:bg-card-dark p-4 rounded-lg shadow border border-border dark:border-border-dark">
          ...
          <p className="mb-4"><span className="font-semibold">Bio:</span> {bio}</p>
          {isEditing ? (
            <>
              <textarea
                value={bio}
              ...
              />
              <button onClick={handleSave} className="bg-button text-white w-full py-2 rounded mb-2">Save</button>
              <button onClick={() => setIsEditing(false)} className="w-full py-2 bg-input dark:bg-layer hover:bg-button-hover rounded">Cancel</button>
            </>
          ) : (
            <button onClick={() => setIsEditing(true)} className="bg-button text-white w-full py-2 rounded">Edit Profile</button>
          )}

          <button onClick={() => deleteUserAccount(authorId)} className="mt-6 text-sm text-red-500 underline">Delete Account</button>
        </aside>
          ...
          <div className="flex border-b border-border dark:border-border-dark mb-6 space-x-6">
            {TABS.map((tab) => (
              <button
                ...
                className={`pb-2 text-lg font-semibold ${activeTab === tab ? "border-b-2 border-button text-button" : "text-secondary dark:text-secondary-dark"}`}
              >
                {tab}
              </button>
            ))}
          ...
          {activeTab === "Bookmarks" && (
            <section className="space-y-4">
              {bookmarks.length > 0 ? bookmarks.map((bookmark) => (
                <div key={bookmark.bookmarkId} className="bg-layer dark:bg-layer-dark p-4 rounded shadow border border-border dark:border-border-dark">
                  <h3 className="text-lg font-bold mb-1">{bookmark.story.title}</h3>
                  <p className="text-sm text-secondary dark:text-secondary-dark mb-2">{bookmark.story.summary || "No summary available"}</p>
                  <button onClick={() => handleReadMore(bookmark.storyId)} className="bg-button text-white px-3 py-1 rounded">Read more</button>
                </div>
          ...
          {activeTab === "Comments" && (
            <section className="space-y-4">
              {comments.length > 0 ? comments.map((comment) => (
                <div key={comment.commentId} className="bg-layer dark:bg-layer-dark p-4 rounded shadow border border-border dark:border-border-dark">
                  <p className="font-semibold">Story: {comment.story.title}</p>
                  <p>{comment.content}</p>
                  <div className="flex space-x-2 mt-2">
                    <button onClick={() => handleReadMore(comment.storyId)} className="bg-button text-white px-3 py-1 rounded">View Story</button>
                    <button onClick={() => handleCommentDelete(comment.commentId)} className="bg-red-600 hover:bg-red-700 text-white px-3 py-1 rounded">Delete</button>
                  </div>
                </div>
          ...
          {activeTab === "Stories" && (
            <section className="space-y-4">
              {stories.length > 0 ? stories.map((story) => (
                <div key={story.storyId} className="bg-layer dark:bg-layer-dark p-4 rounded shadow border border-border dark:border-border-dark">
                  <h3 className="text-lg font-bold mb-1">{story.title}</h3>
                  <p className="text-sm text-secondary dark:text-secondary-dark mb-2">{story.summary || "No summary available"}</p>
                  <div className="flex space-x-2">
                    <button onClick={() => handleReadMore(story.storyId)} className="bg-button text-white px-3 py-1 rounded">Read more</button>
                    <button onClick={() => handleStoryDelete(story.storyId)} className="bg-red-600 hover:bg-red-700 text-white px-3 py-1 rounded">Delete</button>
                  </div>
                </div>
